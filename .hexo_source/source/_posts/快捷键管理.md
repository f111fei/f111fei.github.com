...
tags: 
---
最近接到了要完成软件中的快捷键模块。基本需求就是能做到让用户自由设置。基本之前的所有按键都是写死在代码中的，就像下面这样糟糕：
...
基本上API就是这样，使用很简单，也很灵活。然后是重点`No Code You Say A Diao`
	...
	package
	{
		...
		public class Shortcut
		{
			 ...
			 */
			public static function addBinding(type:String , keyCodeArray:Array):void
			{
				instance.addBinding(type , keyCodeArray);
			}
			 ...
			 * 绑定一个事件类型 与 按键值的映射
			 * @param type  事件类型
			 * @param keyCodeArray 按下的键的数组，数组中的元素对应Keyboard中的常量
			 * 	<br/>如果数组的长度等于1，则 按照对应的键值触发事件
			 * <br/>如果数组的长度大于1，则 按照数组中的第一项对应的键值触发事件，其他的键为触发事件需要按下的键
			 */
			public function addBinding(type:String , keyCodeArray:Array):void
			{
				bindingDic[type] = keyCodeArray;
			}
			...
			private function onKeyDown(event:KeyboardEvent):void
			{
				...
				{
					shortcutMap = shortcutMapList[i] as ShortcutMap;
					if(check(event , shortcutMap.type))
					{
			 ...
			 * 检查事件是否符合类型
			 */
			private function check(event:KeyboardEvent , type:String):Boolean
			{
				var keyCodeArray:Array = bindingDic[type];
...
有一点还没有实现的就是，同一个事件有不同的快捷键。就是说比如保存这个操作，我既可以按Ctrl+S也可以按Ctrl+D，这个其实稍微动一下就行了，关键就是`addBinding`这个函数的第二个参数这是一个按键数组，数组的格式，以及具体的解析我们可以自定义嘛。不过这种需求比较少就懒得写了。
